...
count_sp_information = select_sp_tout_reg |> 
  ...
  # count_sp_information |> distinct(fichier_photo) 
...
for (photos_i in 1:length(img_src)) {
  ...
  image_write(image = img_resized, path = out, quality = 25)
  link_update = data.frame(link = img_src[photos_i], outlink = out)
  # Combine les données 
  dat_all = bind_rows(dat_all, link_update)
}

write.csv2(
  x = dat_all, file = "data/path_imgs.csv", row.names = FALSE)
}
...
# mapview::mapview(regqc, zcol = "MRS_NM_REG")

# runion_raw |> 
#   st_drop_geometry() |> 
#   distinct(MRS_CO_REG, MRS_NM_REG) |> 
#   datapasta::dpasta()
...
# 1. Get unique species with valid images
...
for (i in 1:nrow(unique_species)) {
  ...
  # Get your base64 string here (assuming you read the file and encode it)
  # Let's say it looks like: "data:image/jpeg;base64,/9j/4AAQSkZJRg..."
  base64_string <- base64enc::base64encode(row$fichier_photo)

  # Write it as a global CSS variable
  cat(sprintf('  --img-sp-%s: url("data:image/jpeg;base64,%s");\n', row$id, base64_string))

}


# cat('/* Container Box Settings */
# .taxa-img { 
#   width: 100%; 
#   height: 110px;       
#   min-width: 110px;   
...
#   /* THIS HANDLES LOCKING, CENTERING, AND CROPPING PERFECTLY */
...
# <em style="font-size: 0.75em; color: #555;">%s</em><br>
# <span style="font-size: 0.75em; color: #888;">(%s)</span><br>
```
...
<!--   // Wait until the document and TOC are fully initialized -->
<!--   setTimeout(function() { -->
<!--     // Append the map container to the built-in RMarkdown TOC container -->
<!--     $('#map-to-move').appendTo('#toc'); -->
<!--   }, 100);  -->
<!-- }); -->
<!-- ``` -->
...
<!-- /* Ensure the moved map behaves nicely inside the sidebar */ -->
<!-- #toc #map-to-move { -->
<!--   margin-top: 20px; -->
<!--   padding: 10px; -->
